...
// ... Your other helper components like Note9EditableTable, EditableNoteItem, RenderMuiNoteTable are UNCHANGED .
...
const Note9EditableTable: React.FC<{
  ...
  const isRowEditable = (label: string): boolean =>
    !label.toLowerCase().includes("total");
  const isFinalTotalRow = (label: string): boolean =>
    label.toLowerCase().includes("total trade receivables as on");
            ...
            return (
              <Fragment key={rowIndex}>
                    ...
                    >
                      {isRowEditable(row[0]) ? (
                        <TextField
                          ...
                          size="small"
                          variant="outlined"
                          value={parseNumberForEditing(cell, false)}
                          onChange={(e) =>
                            onCellChange(
                              rowIndex,
                              colIndex + 1,
                ...
                <TableRow>
                  {row.slice(1).map((cell, colIndex) => (
                    <TableCell
                      key={`${rowIndex}-${colIndex}-prev`}
                      align="right"
                      sx={{ p: 0.5, border: "1px solid #ddd" }}
                    >
                      {isRowEditable(row[0]) ? (
                        <TextField
                          fullWidth
                          size="small"
                          variant="outlined"
                          value={parseNumberForEditing(cell, true)}
                          onChange={(e) =>
                            onCellChange(
                              rowIndex,
                              colIndex + 1,
  ...
  return (
    <>
      <TableRow key={item.key}>
        <TableCell
          style={{
            paddingLeft: `${item.children ? 20 : 40}px`,
            fontWeight:
              item.isSubtotal || item.isGrandTotal ? "bold" : "normal",
          }}
        >
          {item.label}
        </TableCell>
...
const NotesEditor: React.FC<NotesEditorProps> = ({
  ...
  const [editableNotes, setEditableNotes] = useState<FinancialNote[]>(() =>
    _.cloneDeep(notes)
  );
  const [selectedNoteId, setSelectedNoteId] = useState<string | null>(null);
  
  // === CHANGE 1: ADD STATE TO TRACK WHICH NOTES THE USER ACTUALLY EDITS ===
  const [touchedNoteNumbers, setTouchedNoteNumbers] = useState<Set<string>>(new Set());
  ...
  // === CHANGE 2: UPDATE ALL CHANGE HANDLERS TO TRACK WHICH NOTE WAS TOUCHED ===
  const handleValueChange = (
    ...
    setTouchedNoteNumbers(prev => new Set(prev).add(String(noteNumber)));
  ...
  const handleTableChange = (
    ...
    const noteNumber = editableNotes[noteIndex]?.noteNumber;
    if (noteNumber) {
        setTouchedNoteNumbers(prev => new Set(prev).add(String(noteNumber)));
    }
  ...
  const handleTableChangeTwoLine = (
    ...
    setEditableNotes((prevNotes) => {
      ...
      if (table?.rows?.[rowIndex]?.[colIndex] !== undefined) {
        ...
        const previousValue = isPrevious
          ? value
          : parseNumberForEditing(currentCell, true) || "";
  ...
  // === CHANGE 3: UPDATE THE SAVE HANDLER TO SEND ONLY THE TOUCHED NOTES ===
  const handleSave = () => {
    const notesToSend = editableNotes.filter(note => 
        touchedNoteNumbers.has(String(note.noteNumber))
    );
  ...
  return (
    <Box sx={{ p: 5, backgroundColor: "grey.100", minHeight: "100vh" }}>
      <AppBar
        ...
        sx={{
          background:
            "linear-gradient(135deg, rgba(69, 75, 248, 1) 0%, rgba(38, 5, 167, 1) 100%)",
        }}
      >
        <Toolbar>
      ...
      <Box sx={{ mt: 3, maxWidth: "100%", mx: "auto" }}>
        {editableNotes
          .filter(
            (note) =>
              !selectedNoteId || String(note.noteNumber) === selectedNoteId
          )
          .map((note) => {
            const originalNoteIndex = editableNotes.findIndex(
              (n) => n.noteNumber === note.noteNumber
            );
            return (
              <Paper
                key={note.noteNumber}
                sx={{ mb: 3, p: 2 }}
                data-note-id={note.noteNumber}
              >
                ...
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell align="right">{amountKeys.amountCurrentKey}</TableCell>
                      <TableCell align="right">{amountKeys.amountPreviousKey}</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {note.content.map((item, itemIndex) => {
                      if (typeof item === "string") {
                        ...
                        if (
                          hierarchicalItem.isNarrative &&
                          hierarchicalItem.isEditableText
                        ) {
                          return (
                              ...
                              <TableCell colSpan={3} sx={{ padding: 2 }}>
                                ...
                                >
                                  ...
                                  <TextField
                                    ...
                                    onChange={(e) => {
                                      // === CHANGE 4: TRACK THE CHANGE IN THIS TEXT FIELD ===
                                      setTouchedNoteNumbers(prev => new Set(prev).add(String(note.noteNumber)));

                                      const newText = e.target.value;
                                      setEditableNotes((prev) => {
                                        const updated = _.cloneDeep(prev);
                                        const targetItem = updated[
                                          originalNoteIndex
                                        ].content[
                                          itemIndex
                                        ] as HierarchicalItem;
                        ...
                        return (
                          <EditableNoteItem
                            key={hierarchicalItem.key}
                            item={hierarchicalItem}
                            path={`${itemIndex}`}
                            onValueChange={(path, field, value) =>
                              handleValueChange(
                                note.noteNumber,
                                path,
                                field,
