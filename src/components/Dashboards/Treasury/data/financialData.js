// Realistic financial data for corporate treasury
export const currentMonth = 'Mar 2024';
...
export const netDebtData = {
  ...
  // Calculated fields
  get cashEquivalents() {
    return this.cashBankBalances + (this.fixedDeposits - this.lienMarkedFDs);
  },
  get netDebt() {
...
export const netDebtMovement = months.map(month => ({
  month,
  openingBalance: Math.floor(Math.random() * 1200000000) + 800000000,
  additions: Math.floor(Math.random() * 300000000) + 50000000,
  repayments: Math.floor(Math.random() * 250000000) + 30000000,
  closingBalance: function() {
    return this.openingBalance + this.additions - this.repayments;
  }
}));

export const borrowingsData = [
  {
    id: 1,
